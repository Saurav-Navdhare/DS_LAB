...
node * vertexSearch(vertex *top,stack *stop,int d,int des){
     ...
     while(a!=NULL){
         if(a->datav==d)    path(top,stop, a->nnext, des);
            a=a->vnext;
     }
}
...
stack* pops(stack *stop,int *n){
    stack *a=stop;
    while(stop){
        if(stop->data < 0)  break;
        stop=stop->snext;
    }
    if(stop==NULL){
        exit(0);
    }
    if(stop->data==-100)    stop->data+=100;
    else    stop->data= -stop->data;
    stop=check(stop);
    *n=stop->data;
    return stop;
}
...
void display(stack *stop){
    ...
    for( j=i-1;j>=0;j--){
        if(arr[j]>=0)   printf("%s ",name[arr[j]]);
    }
}
